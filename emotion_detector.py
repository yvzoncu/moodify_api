...
def fix_embeddings_for_all_songs(batch_size=100):
    conn = get_db_connection()
        ...
        with conn.cursor() as cursor:
            ...
            while True:
                ...
                cursor.execute(
                    """
                    SELECT id, song, artist, song_info, genre, tempo,
                           danceability, energy, acousticness, valence, release_year
                    FROM song_data
                    ORDER BY id
                    LIMIT %s OFFSET %s
                    """,
                ...
                for song in songs:
                    song_dict = dict(song)
                    text = build_embedding_string(song_dict)
                    ...
                    cursor.execute(
                        ...
                        WHERE id = %s
                        """,
                        (new_embedding, song["id"]),
                    )
                conn.commit()
                offset += batch_size
                print(f"Updated embeddings for batch starting at offset {offset}")
